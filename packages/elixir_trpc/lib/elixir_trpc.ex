defmodule ElixirTRPC do
  ...
  `ElixirTRPC` provides a type-safe, schema-driven approach to building JSON-RPC style
  ...
  Define an RPC function using `ElixirTRPC.Function`:
  ...
  `ElixirTRPC` is built on several core components:

  - `ElixirTRPC.Function` - The main macro for defining RPC endpoints with schemas
  - `ElixirTRPC.Conn` - Helpers for JSON response handling
  - `ElixirTRPC.JSON` - Abstraction layer for JSON encoding/decoding
  - `ElixirTRPC.Introspection` - Endpoint for discovering all available RPC contracts

  ## Configuration

  ### JSON Library

  By default, `ElixirTRPC` will use `Jason` if available, falling back to the
  ...
  `ElixirTRPC` can generate a JSON Schema description of all your endpoints for
  ...
  See the [GitHub repository](https://github.com/robsonperassoli/elixir_trpc) for
