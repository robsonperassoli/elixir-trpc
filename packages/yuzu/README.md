...
[![Hex Version](https://img.shields.io/hexpm/v/yuzu.svg)](https://hex.pm/packages/yuzu)
[![Hex Docs](https://img.shields.io/badge/hex-docs-lightgreen.svg)](https://hexdocs.pm/yuzu)
[![License](https://img.shields.io/hexpm/l/yuzu.svg)](https://github.com/robsonperassoli/yuzu/blob/main/LICENSE)

Type-safe, schema-driven JSON-RPC endpoints for Elixir with compile-time validation.

Yuzu brings [tRPC](https://trpc.io/)-style development to the Elixir ecosystem, combining the power of Elixir's concurrency with Zoi's runtime validation to create robust, self-documenting APIs.
...
Create a module using `Yuzu.Function` with `params` and `result` schemas:
...
Yuzu uses [Zoi](https://hex.pm/packages/zoi) for schema definition. Here are common patterns:
...
Yuzu auto-detects your JSON library (prefers Jason, falls back to Elixir's built-in JSON). To explicitly configure:
...
Full documentation is available at [https://hexdocs.pm/yuzu](https://hexdocs.pm/yuzu).
